...
const ExperienceModal: React.FC<ExperienceModalProps> = ({ show, onClose, experienceId }) => {
  ...
  useEffect(() => {
    if (show && experienceId) {
      axios.post("/api/Experience/detail", { id: experienceId })
        .then(res => setExperience(res.data as UpdateExperience))
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-[1100] overflow-auto p-2 sm:p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-full sm:max-w-2xl mx-auto p-2 sm:p-6 relative flex flex-col" style={{ maxHeight: '90vh', minHeight: '400px', overflowY: 'auto' }}>
        {/* Header */}
        ...
        <form className="px-2 sm:px-4 py-2 sm:py-4 flex-1 flex flex-col w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-4 mb-4 w-full">
            <div className="col-span-2">
              ...
              <input className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-sky-200" placeholder="Ingrese el título de la experiencia" value={experience?.nameExperiences || ""} readOnly />
            </div>
            <div className="col-span-2">
              <label className="block font-semibold mb-1">Nombre del establecimiento educativo:</label>
              <input className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-sky-200" placeholder="Ingrese el nombre del establecimiento" value={experience?.name || ""} readOnly />
            </div>
            <div className="col-span-2">
              <label className="block font-semibold mb-1">Nombre Completo del líder:</label>
              <input className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-sky-200" placeholder="Ingrese el nombre completo del líder" value={experience?.nameFirstLeader || ""} readOnly />
            </div>
            <div>
              <label className="block font-semibold mb-1">Departamento:</label>
              <input className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-sky-200" placeholder="Ingrese el departamento" value={experience?.department || ""} readOnly />
            </div>
            <div>
              <label className="block font-semibold mb-1">Fecha:</label>
              <input type="date" className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-sky-200" placeholder="dd/mm/aaaa" value={experience?.developmenttime?.slice(0,10) || ""} readOnly />
            </div>
            <div>
              <label className="block font-semibold mb-1">Municipio:</label>
              <input className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-sky-200" placeholder="Ingrese el municipio" value={experience?.municipality || ""} readOnly />
            </div>
            <div>
            ...
            <div>
              ...
              <input className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-sky-200" placeholder="Ingrese el código DANE" value={experience?.codeDane || ""} readOnly />
            </div>
            ...
            <div className="flex flex-col justify-between">
              ...
              <div className="flex items-center justify-center border border-gray-300 rounded h-32 bg-gray-50">
                {experience?.urlPdf ? (
                  <a href={experience.urlPdf} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">
                    {experience.urlPdf}
                  </a>
                ) : (
            ...
            <div>
              ...
              <input className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-sky-200" placeholder="Ingrese el estado actual" value={experience?.stateId || ""} readOnly />
              
              <label className="block font-semibold mb-1 mt-3">Enlace:</label>
              <div className="flex items-center">
                <input className="w-full border border-gray-300 rounded-l px-3 py-2 focus:outline-none focus:ring-2 focus:ring-sky-200" placeholder="Adjuntar Enlaces" value={experience?.urlLink || ""} readOnly />
